...
import {
  ...
  TrendingUp, Home, ShoppingBag,
} from 'lucide-react';
import { api } from '@/lib/api-client';

// All portal user roles managed on this page
type PortalRole = 'writer' | 'agent' | 'seller';

interface PortalUser {
  _id: string; name: string; email: string;
  role: PortalRole; createdAt: string;
...
const ROLE_CONFIG: Record<PortalRole, { label: string; emoji: string; description: string; color: string }> = {
  writer: { label: 'Content Writer', emoji: '✍️', description: 'Create & manage blog articles',  color: 'bg-emerald-600' },
  agent:  { label: 'Agent',          emoji: '🏠', description: 'List & manage properties',       color: 'bg-blue-600'    },
  seller: { label: 'Seller',         emoji: '🏷️', description: 'Submit own property listings',   color: 'bg-amber-600'   },
};
...
export default function AdminWritersPage() {
  ...
  const [tab,        setTab]        = useState<PortalRole>('writer');
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([adminList, blogData, propData]) => {
        // Filter to portal roles only (writer, agent, seller)
        const portalUsers = (Array.isArray(adminList) ? adminList : [])
          .filter(u => ['writer', 'agent', 'seller'].includes(u.role)) as PortalUser[];
        ...
        const enriched: PortalUserWithStats[] = portalUsers.map(u => {
          ...
          if (u.role === 'seller') {
            const mine       = props.filter(p => p.submittedBy === u._id);
  ...
  const byRole = (r: PortalRole) => users.filter(u => u.role === r);
  const filtered = byRole(tab);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const newUser = await api.post<PortalUser>('/api/admins', {
      ...
      setShowForm(false);
      flash(`${ROLE_CONFIG[form.role].label} "${form.name}" created.`);
  ...
  // Summary card data
  const summaryCards = (Object.keys(ROLE_CONFIG) as PortalRole[]).map(r => ({
    label: `${ROLE_CONFIG[r].label}s`,
    value: byRole(r).length,
    views: byRole(r).reduce((s, u) => s + u.totalViews, 0),
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">Manage writers, agents, and sellers</p>
        </div>
      ...
      {!loading && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {summaryCards.map(({ label, value, views, color }) => (
            <div key={label} className="bg-white rounded-xl shadow-sm p-4">
              ...
              <p className="text-xs text-slate-400 mt-1">{views.toLocaleString()} total views</p>
            </div>
          ))}
        </div>
      )}
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border p-6">
          <h3 className="font-bold text-[#1a2e5a] text-lg mb-4 flex items-center gap-2">
            <UserPlus size={18} className="text-red-700" /> Create Portal Account
          </h3>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <div className="relative">
                <input type="email" required value={form.email}
                  onChange={e => { setForm(f => ({ ...f, email: e.target.value })); setEmailError(e.target.value ? validateEmail(e.target.value) : ''); }}
                  placeholder="sara@gmail.com"
                  className={`w-full border-2 rounded-lg px-3 py-2.5 pr-9 text-sm outline-none ${emailError ? 'border-red-400' : form.email && !emailError ? 'border-emerald-400' : 'focus:border-red-500'}`} />
            ...
            <div>
              ...
              <div className="relative">
                <input type="password" required value={form.confirmPassword}
                  ...
                  className={`w-full border-2 rounded-lg px-3 py-2.5 pr-9 text-sm outline-none ${form.confirmPassword && form.password !== form.confirmPassword ? 'border-red-400' : form.confirmPassword && form.password === form.confirmPassword ? 'border-emerald-400' : 'focus:border-red-500'}`} />
            ...
            {/* Role picker — all 3 portal roles */}
            <div className="md:col-span-2">
              ...
              <div className="grid grid-cols-3 gap-3">
                {(Object.keys(ROLE_CONFIG) as PortalRole[]).map(role => (
                  <label key={role}
                    className={`flex items-start gap-3 p-4 border-2 rounded-xl cursor-pointer transition-all ${form.role === role ? 'border-red-500 bg-red-50' : 'border-slate-200 hover:border-slate-300'}`}>
                    <input type="radio" name="portalRole" value={role} checked={form.role === role}
                      onChange={() => setForm(f => ({ ...f, role }))} className="accent-red-700 mt-0.5" />
                    <div>
                      <p className="font-bold text-[#1a2e5a] text-sm">{ROLE_CONFIG[role].label}</p>
                      <p className="text-xs text-slate-400 mt-0.5">{ROLE_CONFIG[role].description}</p>
                    </div>
                  </label>
                ))}
              </div>
            ...
            <div className="md:col-span-2 flex gap-3 pt-2 border-t">
              ...
              <button type="button" onClick={() => { setShowForm(false); setError(''); setEmailError(''); }}
                className="px-6 py-2.5 rounded-lg font-semibold text-sm text-slate-600 bg-slate-100 hover:bg-slate-200">
                Cancel
              </button>
            </div>
          </form>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="flex">
            {(Object.keys(ROLE_CONFIG) as PortalRole[]).map(r => (
              <button key={r} onClick={() => setTab(r)}
                className={`px-5 py-3.5 text-sm font-semibold transition-colors ${tab === r ? 'text-red-700 border-b-2 border-red-700 bg-red-50/50' : 'text-slate-500 hover:text-slate-700'}`}>
                {ROLE_CONFIG[r].emoji} {ROLE_CONFIG[r].label}s ({byRole(r).length})
              </button>
            ))}
          </div>
          <div className="flex items-center gap-1 pr-4">
            <button onClick={() => setViewMode('list')}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${viewMode === 'list' ? 'bg-slate-200 text-slate-700' : 'text-slate-400 hover:text-slate-600'}`}>
              List
            </button>
            <button onClick={() => setViewMode('performance')}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors flex items-center gap-1 ${viewMode === 'performance' ? 'bg-slate-200 text-slate-700' : 'text-slate-400 hover:text-slate-600'}`}>
        ...
        : filtered.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-slate-500 font-semibold">No {ROLE_CONFIG[tab].label}s yet</p>
            <button onClick={() => { setForm(f => ({ ...f, role: tab })); setShowForm(true); }}
              className="text-red-700 text-sm font-semibold hover:underline mt-2 inline-block">
              Create one →
            </button>
          </div>
        ) : viewMode === 'list' ? (
          <div className="divide-y">
            {filtered.map(u => (
              <div key={u._id} className="px-5 py-4 flex items-center gap-4 hover:bg-slate-50 transition-colors">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold shrink-0 text-sm ${ROLE_CONFIG[u.role].color}`}>
                ...
                <div className="hidden sm:flex items-center gap-4 text-xs text-slate-500 mr-2">
                  <div className="text-center">
                    ...
                    <p className="text-[10px] uppercase">{u.role === 'writer' ? 'Articles' : 'Props'}</p>
                  </div>
                  <div className="text-center">
                ...
                <div className="flex items-center gap-1 text-xs text-slate-400 mr-2">
                  <Clock size={11}/>{u.lastLogin ? new Date(u.lastLogin).toLocaleDateString() : 'Never'}
                </div>
              ...
              <tbody className="divide-y">
                {sorted.map((u, i) => (
                  <tr key={u._id} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs shrink-0 ${ROLE_CONFIG[u.role].color}`}>{u.name[0]?.toUpperCase()}</div>
                    ...
                    <td className="px-5 py-4 text-center">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${u.active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                        {u.active ? '● Active' : '○ Inactive'}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center gap-2 justify-end">
                        <button onClick={() => handleToggle(u)}
                          className={`px-2 py-1 rounded text-xs font-semibold transition-colors ${u.active ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' : 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'}`}>
