...
import { Plus, Trash2, Search, MapPin, Eye, Pencil, Clock, CheckCircle, XCircle } from 'lucide-react';
...
export default function DashboardProperties() {
  ...
  const deleteProperty = async (p: any) => {
    ...
    try {
      ...
      setProperties(ps => ps.filter(x => x._id !== p._id));
    } catch (e: any) {
  ...
  // Sellers can delete their own; admins always can
  const canDelete  = (p: any) =>
    can(role, 'deleteAnyProperty') || (isSeller && p.submittedBy === currentUser.id);

  // Pending count badge for admins
  const pendingCount = !isSeller ? properties.filter(p => p.approvalStatus === 'pending').length : 0;

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-extrabold text-[#1a2e5a] flex items-center gap-3">
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b">
            <tr>
              ...
              <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase hidden lg:table-cell">Area</th>
              <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase hidden md:table-cell">Status</th>
              <th className="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Review</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              const areaDisplay = p.area
                ? formatArea(p.area, (p.areaUnit as AreaUnit) || 'sqft')
                : '—';
              return (
                <tr key={p._id} className="hover:bg-slate-50">
                  ...
                  <td className="px-5 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${ab.cls}`}>{ab.label}</span>
                  </td>
                  {canFeature && (
                    <td className="px-5 py-4">
