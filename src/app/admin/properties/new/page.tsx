  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 bg-red-700 hover:bg-red-600 text-white px-5 py-2.5 rounded-lg font-semibold text-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
            ...
            <div>
              ...
              <textarea rows={4} className="w-full border-2 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-red-500 resize-none"
                placeholder="Describe the property..." value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
            </div>
            ...
            <div>
              ...
              <select className="w-full border-2 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-red-500" value={form.agentId} onChange={e => setForm(f => ({ ...f, agentId: e.target.value }))}>
                <option value="">-- Select Agent --</option>
                {agents.map(a => <option key={(a as any)._id} value={(a as any)._id}>{a.name}</option>)}
              </select>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select className="w-full border-2 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-red-500" value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))}>
                  <option value="residential">Residential</option><option value="commercial">Commercial</option><option value="office">Office</option><option value="plot">Plot</option>
                </select>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
            ...
            <label className="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 hover:border-red-400 rounded-xl p-6 cursor-pointer bg-slate-50 hover:bg-red-50 group">
              <input type="file" accept="image/*" multiple className="hidden" onChange={e => {
                Array.from(e.target.files || []).forEach(file => {
                  const reader = new FileReader();
                  reader.onload = ev => setImages(prev => [...prev.filter(Boolean), ev.target?.result as string]);
                  reader.readAsDataURL(file);
                });
                e.target.value = '';
              }} />
              <Plus size={24} className="text-red-700 mb-2" />
              <p className="font-semibold text-slate-600 text-sm">Click to upload images</p>
              <p className="text-xs text-slate-400 mt-1">JPG, PNG, WEBP. Multiple files allowed.</p>
            </label>
            <div>
              ...
              {images.filter(img => !img.startsWith('data:')).map((img, i) => (
                <div key={i} className="flex gap-2 mb-2">
                  <input type="url" className="flex-1 border-2 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-red-500" placeholder="https://..."
                    value={img} onChange={e => { const a = [...images]; a[images.findIndex((im, j) => !im.startsWith('data:') && images.filter((m, k) => !m.startsWith('data:') && k < j).length === i)] = e.target.value; setImages(a); }} />
                  <button onClick={() => setImages(images.filter(im => im !== img))} className="p-2 text-red-500 hover:bg-red-50 rounded"><X size={16} /></button>
                </div>
              ))}
              <button onClick={() => setImages([...images, ''])} className="flex items-center gap-2 text-red-700 font-semibold text-sm hover:underline mt-1"><Plus size={16} /> Add URL</button>
            </div>
            {images.filter(Boolean).length > 0 && (
              <div className="grid grid-cols-3 gap-2">
                {images.filter(Boolean).map((img, i) => (
                  <div key={i} className="relative group aspect-video">
                    <img src={img} className="w-full h-full object-cover rounded-lg" alt="" onError={e => (e.currentTarget.parentElement!.style.display = 'none')} />
                    <button onClick={() => setImages(images.filter((_, j) => j !== i))}
                      className="absolute top-1 right-1 w-6 h-6 bg-red-600 text-white rounded-full items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity flex"><X size={12} /></button>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" checked={form.featured} onChange={e => setForm(f => ({ ...f, featured: e.target.checked }))} className="w-5 h-5 accent-red-700" />
