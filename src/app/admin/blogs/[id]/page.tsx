...
import { Save, ArrowLeft, Eye, Plus, X, Image as ImageIcon, Search, Loader2, Upload, MapPin, Building2, CheckCircle } from 'lucide-react';
...
export default function EditBlogPage() {
    ...
    api.get<any>(`/api/blogs/${id}`)
      .then(blog => {
        setForm({
          ...
          image:          blog.image          || '',
          images:         [...(blog.images    || []), '', '', '', ''].slice(0, 4) as string[],
  ...
  const handleSave = async (publish?: boolean) => {
    ...
    } catch (e: any) { setError(e.message || 'Failed to update blog'); }
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm truncate max-w-xs">{form.title || 'Untitled'}</p>
          </div>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={() => handleSave(form.published ? undefined : false)} disabled={saving}
          ...
          <button onClick={() => handleSave(!form.published)} disabled={saving || !form.title}
      ...
      <div className="mb-4 flex items-center gap-3">
        <span className={`px-3 py-1 rounded-full text-xs font-bold ${form.published ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
          {form.published ? '● Published' : '○ Draft'}
        </span>
        <Link href={`/blog/${form.title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`}
          target="_blank" className="text-xs text-blue-600 hover:underline font-medium">
          View on site →
        </Link>
      </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">{getMetaTitle().length}/60</p>
              </div>
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">{getMetaDesc().length}/160</p>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="border rounded-xl p-4 bg-slate-50">
                  <p className="text-blue-700 text-base font-medium line-clamp-1">{getMetaTitle() || 'Blog Title'}</p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-5 space-y-4 border-l-4 border-blue-400">
            ...
            <div className="space-y-3">
              <div>
                ...
                {form.areaSlug && <p className="text-xs text-slate-400 mt-1">/blogs/areas/<strong>{form.areaSlug}</strong></p>}
              </div>
              <div>
                ...
                <input type="text" placeholder="e.g. DHA Karachi" value={form.schemeLabel}
                  ...
                  className="w-full border-2 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-blue-400" />
                {form.schemeSlug && <p className="text-xs text-slate-400 mt-1">/blogs/schemes/<strong>{form.schemeSlug}</strong></p>}
              </div>
            </div>
          </div>
