      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            {agents.filter((a) => a.active).length} active agents
          </p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {agents.map((agent) => (
          <div key={agent.id} className="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            {/* Banner + avatar wrapper — relative so avatar can overlap */}
            <div className="relative h-24 bg-gradient-to-r from-[#0f1e3d] to-[#1a2e5a]">
              {/* Active badge */}
              <div className={`absolute top-3 right-3 px-2 py-1 rounded-full text-xs font-bold ${
                agent.active ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"
              }`}>
                {agent.active ? "Active" : "Inactive"}
              </div>

              {/* Avatar — absolute to banner, bottom-left, overflows downward */}
              <img
                src={agent.image}
                alt={agent.name}
            ...
            {/* Card body — top padding clears the overhanging avatar */}
