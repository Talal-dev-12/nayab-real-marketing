...
export default function AdminAgents() {
  ...
  const deleteAgent = async (a: Agent) => {
    ...
    await api.delete(`/api/agents/${(a as any)._id}`);
    setAgents(as => as.filter(x => (x as any)._id !== (a as any)._id));
  };

  const toggleActive = async (a: Agent) => {
    const updated = await api.put<Agent>(`/api/agents/${(a as any)._id}`, { active: !a.active });
    setAgents(as => as.map(x => (x as any)._id === (updated as any)._id ? updated : x));
  };
            ...
            <div key={(agent as any)._id} className="bg-white rounded-xl shadow-sm overflow-hidden">
              ...
              <div className="px-5 pb-5 pt-14">
                ...
                <div className="space-y-1.5 mb-4">
                  <div className="flex items-center gap-2 text-xs text-slate-600"><Phone size={12} className="text-red-700" /> {agent.phone}</div>
                  <div className="flex items-center gap-2 text-xs text-slate-600"><Mail size={12} className="text-red-700" /> {agent.email}</div>
                </div>
                <div className="flex items-center justify-between pt-3 border-t">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => deleteAgent(agent)} className="p-1.5 rounded hover:bg-red-50 text-red-600"><Trash2 size={14} /></button>
                  </div>
                </div>
              </div>
            </div>
