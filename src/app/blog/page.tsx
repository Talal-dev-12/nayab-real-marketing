...
import { defaultBlogs, getFromStorage, STORAGE_KEYS } from '@/lib/data';
...
export default function BlogPage() {
  ...
  useEffect(() => {
    const data = getFromStorage<Blog[]>(STORAGE_KEYS.BLOGS, defaultBlogs).filter((b: Blog) => b.published);
    setBlogs(data);
    setFiltered(data);
    setMounted(true);
  }, []);
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${
                  category === cat ? 'bg-red-700 text-white' : 'bg-white text-slate-600 hover:bg-gray-100 shadow-sm'
                }`}
              >
                {cat === 'all' ? 'All' : cat}
              </button>
