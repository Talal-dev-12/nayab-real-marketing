...
import { defaultProperties, getFromStorage, STORAGE_KEYS } from '@/lib/data';
...
export default function PropertiesPage() {
  ...
  useEffect(() => {
    const data = getFromStorage<Property[]>(STORAGE_KEYS.PROPERTIES, defaultProperties);
    setProperties(data);
    setFiltered(data.filter(p => p.status === 'available'));
    setMounted(true);
  }, []);

  useEffect(() => {
    let result = properties.filter(p => p.status === 'available');
    if (filters.priceType !== 'all') result = result.filter(p => p.priceType === filters.priceType);
    if (filters.type !== 'all') result = result.filter(p => p.type === filters.type);
    if (filters.city !== 'all') result = result.filter(p => p.city.toLowerCase() === filters.city);
